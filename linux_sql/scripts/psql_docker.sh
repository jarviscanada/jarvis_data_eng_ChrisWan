...
case $cmd in
  create)
    ...
    # Start the container
    docker run --name jrvs-psql -e POSTGRES_USER="$db_username" -e POSTGRES_PASSWORD="$db_password" -d \
  ...
  start|stop)
    ...
    if [ $container_status -ne 0 ]; then
      ...
      echo 'Please create the container first before using this command'
      exit 1
    fi
